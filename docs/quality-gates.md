...
| --- | --- | --- |
| Health | `probe_success{job="core_http_health", service="<service>"}` | Debe ser `1` durante la ventana evaluada. |
| Readiness | `probe_success{job="core_http_readiness", service="<service>"}` | Debe ser `1` durante la ventana evaluada. |
| Latency | `probe_duration_seconds{job="core_http_health", service="<service>"}` | Debe ser menor o igual a `0.75`. |
| Metrics | `up{job="core_service_metrics", service="<service>"}` | Debe ser `1` cuando el servicio exponga `/metrics`. |
...
Los servicios core actuales exponen `/api/health`, `/api/ready` y `/api/live`. Para cerrar el gate de metricas, cada servicio deberia exponer `/metrics` en formato Prometheus con al menos:
